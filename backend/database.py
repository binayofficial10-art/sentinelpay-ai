...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    currency TEXT NOT NULL DEFAULT 'INR' CHECK (currency GLOB '[A-Z][A-Z][A-Z]'),
    ...
    provider TEXT NOT NULL CHECK (provider IN ('gemini', 'rule_based_fallback')),
    explanation TEXT NOT NULL,
    ai_explanation TEXT NOT NULL,
    analysis_source TEXT NOT NULL CHECK (analysis_source IN ('gemini', 'rule_based')),
    review_decision TEXT CHECK (review_decision IS NULL OR review_decision IN ('APPROVE', 'BLOCK')),
    reviewed_at TEXT,
    processing_time_ms REAL,
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CHECK (amount = CAST(amount_minor / 100 AS TEXT) || '.' || printf('%02d', amount_minor % 100)),
    CHECK ((analysis_source = 'gemini' AND provider = 'gemini') OR (analysis_source = 'rule_based' AND provider = 'rule_based_fallback'))
)
"""
